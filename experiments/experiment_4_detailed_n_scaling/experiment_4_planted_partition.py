"""
Experiment 4: Size Scaling Analysis on Planted Partition Benchmark

Objective: Determine how each algorithm scales with graph size n on planted partition graphs.
"""
...
def generate_planted_partition_graph(n, seed):
    """Generate Planted Partition graph with given size and seed."""
    for attempt in range(10):
        try:
            G = nx.planted_partition_graph(2, n // 2, 0.5, 0.02, seed=seed)
            for node in G.nodes():
            ...
            continue
    raise RuntimeError(f"Failed to generate planted partition graph for n={n}, seed={seed} after 10 attempts")
...
def run_experiment(csv_path):
    ...
    print(f"Starting Experiment: Planted Partition Size Scaling")
    ...
    for n in N_VALUES:
        ...
        for seed in range(NUM_SEEDS):
            G = generate_planted_partition_graph(n, seed)
...
def main():
    ...
    csv_path = os.path.join(output_dir, f"experiment_4_planted_partition_results_{timestamp}.csv")
