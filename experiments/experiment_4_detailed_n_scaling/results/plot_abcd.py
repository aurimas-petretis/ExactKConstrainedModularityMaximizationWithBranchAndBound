...
def main():
    ...
    files = glob.glob("experiment_4_abcd_results_*.csv")
    if not files:
    ...
    ax1.set_title(f'ABCD Median Time vs Graph Size')
    ax1.set_yscale('log')
    ...
    ax2.set_title(f'ACBD Success Rate vs Graph Size (timeout={timeout:.0f}s)')
    ax2.legend()
    ...
    output_path = "plot_abcd_n_detailed.pdf"
