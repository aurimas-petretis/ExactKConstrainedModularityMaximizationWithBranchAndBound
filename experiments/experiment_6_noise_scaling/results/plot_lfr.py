"""Plot LFR noise scaling results."""
...
def main():
    ...
    files = glob.glob("experiment_6_lfr_results_*.csv")
    if not files:
    ...
    # Plot 1: Median time vs mu (all runs, timed-out use timeout value)
    ax1 = axes[0]
    ...
    for alg in algorithms:
        ...
        stats = alg_df.groupby('mu')['time'].median()
    ...
    ax1.set_xlabel('Noise (mu)')
    ax1.set_ylabel('Time (seconds)')
    ax1.set_title(f'LFR Median Time vs Noise Between Clusters')
    # ax1.set_yscale('log')
    ax1.legend(loc='upper left')
    ax1.grid(True, alpha=0.3)

    # Plot 2: Success rate vs mu
    ax2 = axes[1]
    for alg in algorithms:
        alg_df = df[df['algorithm'] == alg]
        stats = alg_df.groupby('mu')['success'].mean() * 100
        ax2.plot(stats.index, stats.values, label=alg,
                 color=COLORS.get(alg, 'gray'), marker=MARKERS.get(alg, 'o'), markersize=6)

    ax2.set_xlabel('Noise (mu)')
    ax2.set_ylabel('Success rate (%)')
    ax2.set_title(f'LFR Success Rate vs Noise Between Clusters (timeout={timeout:.0f}s)')
    ax2.legend()
    ...
    output_path = "plot_lfr_noise_scaling.pdf"
