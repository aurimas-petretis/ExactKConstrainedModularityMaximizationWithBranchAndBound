"""Plot LFR degree distribution scaling results."""
...
def main():
    # Find latest LFR results
    files = glob.glob("experiment_3_lfr_results_*.csv")
    if not files:
        print("No LFR results found in results/")
        return
    ...
    # Plot 1: Median time vs tau1 (all runs, timed-out use timeout value)
    ax1 = axes[0]
    for alg in algorithms:
        ...
        stats = alg_df.groupby('tau1')['time'].median()
    ...
    ax1.set_xlabel('Degree distribution exponent (τ₁)')
    ax1.set_ylabel('Time (seconds)')
    ax1.set_title('LFR: Median Time vs Degree Distribution Exponent')
    ax1.set_yscale('log')
    ax1.legend(loc='upper left')
    ax1.grid(True, alpha=0.3)

    # Plot 2: Success rate vs tau1
    ax2 = axes[1]
    for alg in algorithms:
        alg_df = df[df['algorithm'] == alg]
        stats = alg_df.groupby('tau1')['success'].mean() * 100
        ax2.plot(stats.index, stats.values, label=alg,
                 color=COLORS.get(alg, 'gray'), marker=MARKERS.get(alg, 'o'), markersize=6)

    ax2.set_xlabel('Degree distribution exponent (τ₁)')
    ax2.set_ylabel('Success rate (%)')
    ax2.set_title(f'LFR Success Rate vs Degree Distribution Exponent (timeout={timeout:.0f}s)')
    ax2.legend()
    ...
    output_path = "plot_lfr_degree_distribution.pdf"
