"""
Combined plotting script for Greedy Algorithm Iteration Scaling experiment.
...
def load_latest_results(results_dir):
    ...
    pattern = os.path.join(results_dir, "experiment_2_lfr_*.csv")
...
def plot_combined(df, output_dir):
    ...
    # Get unique iteration and mu values sorted
    iterations = sorted(df_success['iterations'].unique())
    mu_values = sorted(df_success['mu'].unique())

    # Colors and markers for different mu values
    ...
    for idx, mu in enumerate(mu_values):
        df_mu = df_success[df_success['mu'] == mu]
        ...
        for it in iterations:
            mods = df_mu[df_mu['iterations'] == it]['modularity'].values
            gaps = df_mu[df_mu['iterations'] == it]['gap'].values
        ...
        all_mean_opts[mu] = df_mu['optimal_modularity'].mean()
        ...
        ax1.axhline(y=all_mean_opts[mu], color=colors[idx % len(colors)],
    ...
    for idx, mu in enumerate(mu_values):
        ...
        for it in iterations:
            it_data = df_mu[df_mu['iterations'] == it]
    ...
    # Main title
    fig.suptitle('Greedy Algorithm Performance on LFR Graphs', fontsize=16, y=1.02)

    plt.tight_layout()

    # Save figure
    os.makedirs(output_dir, exist_ok=True)
    pdf_path = os.path.join(output_dir, "greedy_iterations_combined.pdf")
